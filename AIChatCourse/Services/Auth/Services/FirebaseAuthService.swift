...
struct FirebaseAuthService: AuthService {
    ...
    func signInApple() async throws -> (user: UserAuthInfo, isNewUser: Bool) {
        ...
        print("ID Token: \(response.token)")
        print("Nonce: \(response.nonce)")
        let credential = OAuthProvider.credential(providerID: AuthProviderID.apple, idToken: response.token, rawNonce: response.nonce)
